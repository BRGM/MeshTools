...
auto collect_constrained_edges_as_curves(const Triangulated_surface& S,
  ...
  auto collect_vertices_until_corner = [&mesh, &edge_in_curve](
    ...
    auto find_single_exit = [&edge_in_curve, &mesh](Vertex_index v) {
      ...
      //    << std::count_if(begin(edge_in_curve), end(edge_in_curve), [](auto
      //    p) { return p.second; })
...
auto intersection_curves(Triangulated_surface& S1, Triangulated_surface& S2,
                         Output_iterator out) {
  typedef Triangulated_surface Mesh;
  typedef typename Mesh::Vertex_index Vertex_index;
  typedef typename Mesh::Edge_index Edge_index;
...
auto extract_submesh(const Triangulated_surface& mesh, Face_iterator first,
                     Face_iterator last) {
