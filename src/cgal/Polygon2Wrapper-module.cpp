...
struct Polygon {
	...
	Polygon(const CGAL_Polygon& border) :
		boundary{ border }
	{}
	...
	auto is_consistent() const {
        for(auto&& hole: holes) {
            if(! std::all_of(
                    hole.vertices_begin(), hole.vertices_end(),
                    [this](const Point2& P) { 
...
inline auto array_to_point_buffer(py::array_t<double, py::array::c_style>& points)
{
	assert(points.ndim() == 2);
	assert(points.shape(1) == 2);
	static_assert(sizeof(Point2)==2*sizeof(double), "unconsistent sizes in memory");
	return Point2Buffer {
		reinterpret_cast<Point2 *>(points.mutable_data(0, 0)),
		static_cast<std::size_t>(points.shape(0))
	};
}

auto cgal_polygon_from_points_array(py::array_t<double, py::array::c_style>& points, const double squared_threshold)
{
...
auto polygon_has_all_points_inside(const Polygon& polygon, py::array_t<double, py::array::c_style>& points)
{
	auto buffer = array_to_point_buffer(points);
	return std::all_of(
		buffer.begin(), buffer.end(),
		[&polygon](const Point2& P) { return polygon.has_point_inside(P); }
	);
}

auto polygon_has_points_inside(const Polygon& polygon, py::array_t<double, py::array::c_style>& points)
{
	auto buffer = array_to_point_buffer(points);
...
void add_polygon_wrapper(py::module& module)
{
    ...
    .def_property_readonly("coordinates", [](Point2& self) {
        return py_double_array {
            2, reinterpret_cast<double *>(&self)
        };
    })
    ...
    .def(py::init([](py_double_array& points, const double squared_threshold) {
    ...
    .def_property_readonly("points", [](const CGAL_Polygon& self) {
        return py::make_iterator(self.vertices_begin(), self.vertices_end());
    })
    ...
    .def(py::init([](py::list polygons, const double squared_threshold) {
		...
		for (std::size_t k = 1; k != py::len(polygons); ++k) {
			auto hole = polygons[k].cast<py_double_array>();
				...
				cgal_polygon_from_points_array(
					hole, squared_threshold
				)
			);
		}
		return res;
    ...
    .def_property_readonly("holes", [](Polygon& self) {
        return py::make_iterator(begin(self.holes), end(self.holes));
    })
	.def("has_holes", [](Polygon& self) {
		return !self.holes.empty();
	})
	.def("nb_holes", [](Polygon& self) {
		return self.holes.size();
	})
		.def("is_consistent", &Polygon::is_consistent)
	.def("has_point_inside", &Polygon::has_point_inside)
	.def("has_points_inside", [](const Polygon& self, py_double_array& points) {
