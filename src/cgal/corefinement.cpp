...
auto corefine_surfaces(Blob& blob, Mesh& S1, Mesh& S2) {
  ...
  auto add_constraint_map = [](Mesh& mesh) {
    auto result =
        mesh.add_property_map<Edge_index, bool>("e:constrained", false);
  ...
  CGAL::Polygon_mesh_processing::corefine(
      S1, S2, parameters::edge_is_constrained_map(constraints1),
      parameters::edge_is_constrained_map(constraints2), true);
  ...
  std::cout << "Number of blob curves: "
            << blob.curves_factory.already_created() << std::endl;
}
...
auto test() {
  ...
  tm2.add_face(u, v, w);
  tm2.add_face(u, w,
               x);  // beware of face orientation - otherwise face is not added
  Mesh tm3;
...
py::object apply_functor_on_array(Functor F,
  ...
  static_assert(sizeof(Arg) == dim * sizeof(DType),
