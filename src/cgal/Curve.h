...
struct Curve : std::list<Node_type> {
  ...
  bool is_valid() const {
    ...
    for (auto pnode = next(this->begin()); pnode != prev(this->end());
         ++pnode) {
  ...
  auto incident_surfaces() const {
    assert(is_valid());
    return std::make_pair(this->front().info[0].surface,
                          this->front().info[1].surface);
  }
  template <typename Surface>
  auto has_incident_surface(Surface* S) const {
    assert(is_valid());
    return (this->front().info[0].surface == S ||
  ...
  auto share_surface(const Curve& other) const {
    ...
    if (this_surfaces.first == other_surfaces.second)
      return this_surfaces.first;
    if (this_surfaces.second == other_surfaces.first)
      return this_surfaces.second;
    if (this_surfaces.second == other_surfaces.second)
      return this_surfaces.second;
    return nullptr;
  }
