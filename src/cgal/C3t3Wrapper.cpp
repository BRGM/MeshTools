...
typedef ExperimentalOracle::DummyDomain<Kernel, SubdomainIndex,
                                        SurfacePatchIndex>
    Domain;
typedef CGAL::Mesh_domain_with_polyline_features_3<Domain> Mesh_domain;
typedef CGAL::Mesh_triangulation_3<Mesh_domain, Mesh_domain::R,
                                   CGAL::Sequential_tag>::type Triangulation;
...
//{
//
//	std::cout << "Extracting point data from file: " << filename <<
// std::endl; 	std::ifstream file(filename, std::ios_base::in); 	if
// (!file.good()) { 		file.close(); 		std::cerr << "Something
// went wrong in extracting points." << std::endl; 		return -1;
//	}
//	else {
...
//		for (std::vector<SPI>::const_iterator p = patch_indexes.begin();
// p
//!= patch_indexes.end(); ++p) { 			index_dictionnary <<
//!"Index:
//!"
//!<< p->first <<
//"," << p->second << std::endl;
...
//			if (!file.good()) break;
//			patch_distances << P;
//			for (std::vector<SPI>::const_iterator p =
// patch_indexes.begin(); p != patch_indexes.end(); ++p) {
// patch_distances << " "
//<< dtree.squared_distance(P, *p);
//			}
...
auto apply(std::function<Result(Input)> F,
  ...
  if (a.itemsize() == sizeof(Input_type)) {
    ...
    result_shape = std::vector<std::size_t>{info.shape.begin(),
                                            info.shape.begin() + a.ndim() - 1};
  } else {
    throw std::runtime_error("Incompatible size in memory!");
  }
  auto result = Result_array{result_shape};
  // WARNING: If Input_type is not const cast will fail as a.data is const: use
  // a.mutable_data
...
class Id_generator {
 ...
 public:
  Id_generator(T start = 0) : id{start} {}
...
class C3t3Wrapper {
 ...
 protected:
  ...
  void update_info() {
    ...
    const C3t3& complex = c3t3;
    const Triangulation& triangulation = complex.triangulation();
    typedef std::map<typename Triangulation::Vertex_handle, Id_type>
        Vertices_map;
    Vertices_map vmap;
    {
      Id_generator<Id_type> genid;
      for (auto p = triangulation.finite_vertices_begin();
           p != triangulation.finite_vertices_end(); ++p) {
        vmap[p] = genid();
      }
    ...
    std::transform(triangulation.finite_vertices_begin(),
    ...
    //	auto P = p.point();
    //	std::copy(P.cartesian_begin(), P.cartesian_end(),
    ...
    std::transform(
        ...
        [&vmap](const Complex_cell& cell) -> Simplex {
          return Simplex{{vmap.at(cell.vertex(0)), vmap.at(cell.vertex(1)),
                          vmap.at(cell.vertex(2)), vmap.at(cell.vertex(3))}};
        });
    std::transform(complex.cells_in_complex_begin(),
                   complex.cells_in_complex_end(),
    ...
    std::transform(complex.facets_in_complex_begin(),
                   complex.facets_in_complex_end(), std::back_inserter(facets),
 ...
 public:
  ...
  auto vertices_buffer() {
    // std::cerr << "Size: " << c3t3.triangulation().number_of_vertices() << "
    // vs " << vertices.size() << std::endl;
    ////std::cerr << "Size: " << c3t3.triangulation().number_of_vertices() << "
    /// vs " << vertices_tmp.size() << std::endl;
    return Vertices_buffer{
        ArrayWrapper::wrap(vertices.front().data(), vertices.size())};
    // return Vertices_buffer{ ArrayWrapper::wrap(vertices_tmp) };
  }
  auto cells_buffer() {
    // std::cerr << "Cell size: " << c3t3.number_of_cells() << " vs " <<
    // cells.size() << std::endl;
    return Cells_buffer{ArrayWrapper::wrap(cells.front().data(), cells.size())};
  }
  auto domain_tags_buffer() {
    return Domain_tag_buffer{
        ArrayWrapper::wrap(domain_tags.data(), domain_tags.size())};
  }
  auto facets_buffer() {
    return Facets_buffer{
        ArrayWrapper::wrap(facets.front().data(), facets.size())};
  }
  auto facet_tags_buffer() {
    return Facet_tag_buffer{
        ArrayWrapper::wrap(facet_tags.front().data(), facet_tags.size())};
  }
  auto squared_distance_functor(Id_type f1, Id_type f2) const {
    ...
    return [tree, spi](const Pt& P) -> double {
      static_assert(sizeof(DTree::Point) == sizeof(Pt),
                    "Incompatible Point types !");
      return tree->squared_distance(reinterpret_cast<const DTree::Point&>(P),
                                    spi);
    };
  }
  auto squared_distances(Id_type f1, Id_type f2,
                         py::array_t<double, py::array::c_style> pts) {
    auto distance_functor =
        std::function<double(const Pt&)>{squared_distance_functor(f1, f2)};
...
struct Foo {
  ...
  Foo() : a{{2, 3}}, b{{5, 2}}, c{{4, 4}} {
    // Hack to make the array const from:
    // https://github.com/pybind/pybind11/issues/481
    reinterpret_cast<py::detail::PyArray_Proxy*>(c.ptr())->flags &=
...
void f(py::array_t<double, py::array::c_style> a) {
  ...
  if (info.shape.back() != 3) throw std::runtime_error("3D points only!");
...
double f1(const Pt& P) { return P.x + P.y + P.z; }

int f2(const Pt& P) { return static_cast<int>(P.x + P.y + P.z); }
...
PYBIND11_MODULE(C3t3Wrapper, module) {
  ...
  py::class_<C3t3Wrapper>(module, "C3t3")
      ...
      .def("vertices", &C3t3Wrapper::vertices_buffer,
           py::return_value_policy::reference_internal)
      .def("cells", &C3t3Wrapper::cells_buffer,
           py::return_value_policy::reference_internal)
      .def("domain_tags", &C3t3Wrapper::domain_tags_buffer,
           py::return_value_policy::reference_internal)
      .def("facets", &C3t3Wrapper::facets_buffer,
           py::return_value_policy::reference_internal)
      .def("facet_tags", &C3t3Wrapper::facet_tags_buffer,
           py::return_value_policy::reference_internal)
  ...
  //	// IMPROVE: C++17: type deduction on std::function, recast_function is
  // no longer needed 	return apply(recast_function(f1), a);
  //});

  // module.def("f2", [](py::array_t<double, py::array::c_style> a) {
  //	// IMPROVE: C++17: type deduction on std::function, recast_function is
  // no longer needed 	return apply(recast_function(f2), a);
  //});
  ...
  py::class_<Foo>(module, "Foo")
      ...
      .def_readonly(
          "b", &Foo::b)  // you can change the content of b but not the property
                         // : shape, dtype, allocated chunk of memory
      .def_readonly("c", &Foo::c);  // you cannot change the content of c
...
//	//else {
//	//	assert(argc > 0);
//	//	std::cerr << argv[0] << " c3t3file distances_results" <<
// std::endl;
//	//}
//	return 0;
//
