...
template <typename BaseShape,
          typename Kernel = typename CGAL::Kernel_traits<BaseShape>::Kernel>
class DTM {
 public:
 ...
 protected:
  ...
  typedef typename CGAL::AABB_triangle_primitive<Kernel, ShapeIterator>
      Primitive;
  typedef typename CGAL::AABB_traits<Kernel, Primitive> PT;  // Primitive Traits
  typedef CGAL::AABB_tree<PT> Tree;
  typedef typename Tree::template Intersection_and_primitive_id<Line>::Type
 ...
 public:
  DTM(DTM&& other) noexcept : patches{std::move(other.patches)}, tree{} {
    rebuild_tree();
  }
  DTM(const Patches& all_shapes) : patches{all_shapes}, tree{} {
    rebuild_tree();
  }
  DTM(Patches&& all_shapes) noexcept : patches{std::move(all_shapes)}, tree{} {
    rebuild_tree();
  }
...
auto build_dtm_from_triangulation(PointIterator first, PointIterator last) {
  ...
  for (auto p = triangulation.finite_faces_begin();
       p != triangulation.finite_faces_end(); ++p) {
    triangles.emplace_back(Triangle(
        p->vertex(0)->point(), p->vertex(1)->point(), p->vertex(2)->point()));
  }
