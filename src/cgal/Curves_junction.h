...
struct Curves_junction : std::pair<On_curve_constraint<Curve_type>,
                                   On_curve_constraint<Curve_type>> {
  ...
  Curves_junction(On_curve&& c1, On_curve&& c2)
      : std::pair<On_curve, On_curve>{std::forward<On_curve>(c1),
  ...
  bool is_valid() const noexcept {
    ...
    if (!(this->first.position->point() == this->second.position->point()))
      return false;
    return true;
  }
};
