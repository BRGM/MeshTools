...
class PyBufferWrapper {
 ...
 public:
  ...
  auto buffer_info() -> py::buffer_info {
    return py::buffer_info(static_cast<T*>(wrapper.pointer), sizeof(T),
                           py::format_descriptor<T>::format(), ndim, shape,
                           strides);
  }
  static auto add_buffer_class(py::module& module,
                               const std::string& classname) {
    typedef PyBufferWrapper<T, extend...> Buffer;
    return py::class_<Buffer>(module, classname.c_str(), py::buffer_protocol())
        .def_buffer([](Buffer& buffer) -> py::buffer_info {
          return buffer.buffer_info();
        });
  }
};
