...
auto mesh_as_arrays(const Mesh& mesh) {
  ...
  auto triangles = py::array_t<New_index, py::array::c_style>{
      {static_cast<std::size_t>(mesh.number_of_faces()),
...
void copy_boundary_to_surface_mesh(
    const C3T3& c3t3, typename C3T3::Subdomain_index sd_index, Mesh& mesh,
  ...
  for (typename C3T3::Facets_in_complex_iterator
    ...
    typename C3T3::Subdomain_index opp_sd =
        c3t3.subdomain_index(fit->first->neighbor(fit->second));
    ...
    if (((cell_sd == sd_index) == (fit->second % 2 == 1)) ==
    ...
    if (new_face == Mesh::null_face()) {
      ++failure_on_first_insertion;  // std::cerr << "First face insertion
                                     // failed";
  ...
  py::print("Failures:", failure_on_first_insertion,
            failure_on_second_insertion);
}

template <typename Mesh, typename C3T3>
auto collect_all_boundaries_as_surface_meshes(
    const C3T3& c3t3, bool normals_point_outside_of_the_subdomain = true) {
  ...
  for (auto&& sdi : indices) {
    auto mesh = Mesh{};
    copy_boundary_to_surface_mesh(c3t3, sdi, mesh,
                                  normals_point_outside_of_the_subdomain);
    meshes.append(mesh);
  }
  return meshes;
