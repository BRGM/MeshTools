...
template <typename Target_type, typename Source_type = Target_type,
          int nbcols = 1>
struct XArrayWrapper {
  ...
  XArrayWrapper(XArrayWrapper&& wrapper) : data{wrapper.data}, n{wrapper.n} {}
  explicit XArrayWrapper(py::array_t<Source_type, py::array::c_style> a)
      : data{nullptr}, n{0} {
    static_assert(sizeof(Target_type) == nbcols * sizeof(Source_type),
...
void add_epick_wrapper(py::module& module) {
  ...
  py::class_<Polyline>(module, "Polyline", py::buffer_protocol())
      ...
      .def(py::init([](py::array_t<double, py::array::c_style> points) {
        ...
        return std::make_unique<Polyline>(std::begin(wrapper),
                                          std::end(wrapper));
      }))
      .def(
          "__iter__",
      ...
      .def_buffer([](Polyline& self) -> py::buffer_info {
        return py::buffer_info(
            reinterpret_cast<double*>(self.data()), sizeof(double),
      ...
      .def(
          ...
          [](Polyline& self) {
            ...
            return double_array{
                ...
                double_array{}  // used to create a handle so that the returned
                                // array is effectively a view
            };
          },
          py::keep_alive<0, 1>())
  ...
  py::class_<Triangulated_surface>(module, "TSurf")
      ...
      .def(py::init(
          [](py::array_t<double, py::array::c_style | py::array::forcecast>
            ...
            auto triangles_wrapper =
                array_wrapper<3, const std::array<std::size_t, 3>>(
                    remove_forcecast(triangles));
      ...
      .def(
          "as_arrays",
          [](const Triangulated_surface& self) { return mesh_as_arrays(self); })
      .def("submeshes",
           [](const Triangulated_surface& self) {
             std::map<Triangulated_surface::Face_index,
                      Triangulated_surface::faces_size_type>
                 facemap_container;
             boost::associative_property_map<
                 std::map<Triangulated_surface::Face_index,
                          Triangulated_surface::faces_size_type>>
                 facemap(facemap_container);

             const size_t num_submeshes =
                 CGAL::Polygon_mesh_processing::connected_components(self,
                                                                     facemap);
             py::list submeshes;

             if (num_submeshes ==
                 1) {  // No need to extract, skip the hard work.
      ...
      .def("face_centers",
           [](const Triangulated_surface& self) {
             auto centers = py::array_t<double, py::array::c_style>{
                 {static_cast<std::size_t>(self.number_of_faces()),
             ...
             for (auto&& f : self.faces()) {
               fv.clear();
               for (auto&& v :
                    CGAL::vertices_around_face(self.halfedge(f), self)) {
                 fv.emplace_back(v);
               }
               assert(fv.size() == 3);
               *pC = CGAL::centroid(self.point(fv[0]), self.point(fv[1]),
                                    self.point(fv[2]));
               ++pC;
             }
             return centers;
      ...
      .def("to_off",
           [](const Triangulated_surface& self, py::str& filename) {
             auto os = std::ofstream{filename};
             CGAL::write_off(os, self);
             os.close();
           })
      .def("remove_faces",
           [](Triangulated_surface& self,
              py::array_t<bool, py::array::c_style> where) {
             assert(where.ndim() == 1);
      ...
      .def("is_closed",
           [](const Triangulated_surface& self) {
             return CGAL::is_closed(self);
           })
      .def_property_readonly("nb_vertices",
                             &Triangulated_surface::number_of_vertices)
      ...
      .def("join", [](Triangulated_surface& self,
                      const Triangulated_surface& other) { self.join(other); })
      .def(
          "orient",
          [](Triangulated_surface& self, bool outward_orientation) {
      ...
      .def("self_intersections", [](const Triangulated_surface& self) {
        ...
        for (auto&& faces : self_intersections) {
          result.append(py::make_tuple(static_cast<std::size_t>(faces.first),
    ...
    for (const auto& he : S.halfedges()) {
      if (S.is_border(he)) {
        ...
        CGAL::Polygon_mesh_processing::triangulate_and_refine_hole(
            S, he, std::back_inserter(patch_faces),
