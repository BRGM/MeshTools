...
class C3t3Wrapper {
 ...
 public:
  ...
  //		static_assert(sizeof(DTree::Point) == sizeof(Pt), "Incompatible
  // Point types !"); 		return
  // tree->squared_distance(reinterpret_cast<const DTree::Point&>(P), spi);
  //	};
  //}
  ...
  auto domain_tags_buffer() const {
    return BufferInfo<Domain_tag>{domain_tags};
  }
  ...
  const C3t3& get() const { return c3t3; }
};

// template<typename Result, typename Input>
// auto apply(std::function<Result(Input)> F, py::array_t<double,
// py::array::c_style> a)
//{
...
//    // WARNING: If Input_type is not const cast will fail as a.data is const:
//    use a.mutable_data auto begin_input = reinterpret_cast<Input_type
//    *>(a.data(0)); assert(a.nbytes() % sizeof(Input_type) == 0); auto
//    end_input = begin_input + (a.nbytes() / sizeof(Input_type));
...
//    ////std::cerr << "Size: " << c3t3.triangulation().number_of_vertices() <<
//    " vs " << vertices_tmp.size() << std::endl; return Vertices_buffer{
...
//    cells.size() << std::endl; return Cells_buffer{
//    ArrayWrapper::wrap(cells.front().data(), cells.size()) };
//}
// auto domain_tags_buffer() {
//    return Domain_tag_buffer{ ArrayWrapper::wrap(domain_tags.data(),
//    domain_tags.size()) };
//}
// auto facets_buffer() {
//    return Facets_buffer{ ArrayWrapper::wrap(facets.front().data(),
//    facets.size()) };
//}
// auto facet_tags_buffer() {
//    return Facet_tag_buffer{ ArrayWrapper::wrap(facet_tags.front().data(),
//    facet_tags.size()) };
//}
// auto squared_distances(Id_type f1, Id_type f2, py::array_t<double,
// py::array::c_style> pts) {
//    auto distance_functor = std::function<double(const Pt&)>{
//    squared_distance_functor(f1, f2) }; return apply(distance_functor, pts);
//}
//
