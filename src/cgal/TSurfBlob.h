...
struct TSurfBlob {
  ...
  struct Curves_factory {
   ...
   public:
    Curve *new_curve() {
      pointers.emplace_back(std::make_shared<Curve>());
      return pointers.back().get();
    }
    auto already_created() const noexcept { return pointers.size(); }
  };

  Curves_factory curves_factory;
  std::multimap<Surface_intersection, Curve *> intersections;

  auto new_empty_curve() { return curves_factory.new_curve(); }
