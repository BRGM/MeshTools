...
class DistanceTrees {
 ...
 public:
  ...
  /** Return the squared distance between point P and the complex facets of type
   * spi. */
  template <typename SurfacePatchIndex>
  inline FT squared_distance(const Point& P,
                             const SurfacePatchIndex& spi) const;
...
DistanceTrees<Complex>::DistanceTrees(const Complex& complex)
  ...
  TriangleMap patches;
  for (typename Complex::Facets_in_complex_iterator pf =
           complex.facets_in_complex_begin();
  ...
  for (typename TriangleMap::const_iterator ptl = patches.begin();
       ptl != patches.end(); ++ptl) {
  ...
  for (typename TriangleMap::const_iterator ptl = patches.begin();
       ptl != patches.end(); ++ptl) {
    // WARNING all this is supposed to work because we have reserved the good
    // amount of triangles
    //         and no reallocation of memory is supposed to occur
    TriangleIterator p = triangles.end();
    std::copy(ptl->second.begin(), ptl->second.end(),
              std::back_inserter(triangles));
    assert(std::distance(p, (TriangleIterator)triangles.end()) ==
           ptl->second.size());
...
OutputIterator DistanceTrees<Complex>::patch_indexes(OutputIterator out) const {
  for (typename std::map<Surface_patch_index,
                         std::shared_ptr<Tree> >::const_iterator p =
           patchtrees.begin();
       p != patchtrees.end(); ++p) {
    *out = p->first;
    ++out;
...
template <typename Complex>
typename DistanceTrees<Complex>::FT DistanceTrees<Complex>::squared_distance(
    const Point& P) const {
...
template <typename SurfacePatchIndex>
typename DistanceTrees<Complex>::FT DistanceTrees<Complex>::squared_distance(
    const Point& P, const SurfacePatchIndex& spi) const {
