...
auto sphere_torus_intersection(std::size_t nb_points_on_circle,
                               const double y) {
  const double r = 5. / 3.;
  ...
  for (; nb_points_on_circle != 0; --nb_points_on_circle) {
    ...
    assert(fabs(torus_function(x, y, z)) + fabs(sphere_function<3>(x, y, z)) <
           1E-14);
    if (fabs(torus_function(x, y, z)) + fabs(sphere_function<3>(x, y, z)) >
        1E-14)
  ...
  Point first_point =
      pl.front();  // pl.emplace_back(pl.front()) fails if pl capacity is
                   // saturated (e.g. with pl.reserve(nb_points_on_circle))
...
auto sharp_edges(std::size_t nb_points_on_circle = 1000) {
  ...
  polylines.emplace_back(sphere_torus_intersection(nb_points_on_circle, h));
  polylines.emplace_back(sphere_torus_intersection(nb_points_on_circle, -h));
  return polylines;
}
...
py::list mesh_implicit_domains_boundaries() {
  ...
  auto domain = Mesh_domain{Mesh_domain::create_implicit_mesh_domain(
  ...
  //        CGAL::parameters::bounding_object =
  ...
  static_assert(std::is_same<CGAL::Tag_true,
                             typename decltype(domain)::Has_features>::value,
                "Has feature!");
  ...
  C3t3 c3t3 = CGAL::make_mesh_3<C3t3>(
      domain, criteria, no_exude(), no_perturb()
      //, features() // this can be set manually but is true with
      // Mesh_domain_with_polyline_features_3 , manifold() // any of the two
      // manifolds options the mesh computation does not end ,
  ...
  return collect_all_boundaries_as_surface_meshes<
      CGAL::Surface_mesh<typename CGAL::Epick::Point_3>>(c3t3);
}
