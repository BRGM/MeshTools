...
struct Constrained_node {
  ...
  Curve_node() = delete;
  Curve_node(Info i1, Info i2) : degree{2}, info{{i1, i2, Info{}}} {
    assert(i1.surface != i2.surface);
  }
  Curve_node(Info i1, Info i2, Info i3) : degree{3}, info{{i1, i2, i3}} {
    assert(i1.surface != i2.surface && i1.surface != i3.surface &&
           i2.surface != i3.surface);
  }
  Curve_node(Curve_node cn, Info i)
      : degree{3}, info{{cn.info[0], cn.info[1], i}} {
    assert(cn.degree == 2);
    assert(i.surface != cn.info[0].surface && i.surface != cn.info[1].surface);
  }
