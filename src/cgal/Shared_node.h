...
struct Shared_node {
  struct Info {
    ...
    Surface_base_point& base_point() { return point(); }
    const Surface_base_point& base_point() const { return point(); }
    Surface_constraint& constraint() { return point().constraint; }
    const Surface_constraint& constraint() const { return point().constraint; }
    auto is_valid() const {
      return surface &&
             (surface->is_valid(index) || index == surface->null_vertex());
    }
    auto is_on_surface() const {
      assert(surface);
      return surface->is_valid(index);
    }
    Info() : surface(nullptr), index{} {}
    Info(Surface* S) : surface(S), index{surface->null_vertex()} {
      assert(!is_on_surface());
    }
    Info(Surface* S, Vertex_index v) : surface(S), index{v} {
      assert(is_on_surface());
    }
  };
  std::array<Info, 3> info;
  Shared_node() = delete;
  Shared_node(Info i1, Info i2) : info{{i1, i2, Info{}}} {
    assert(i1.surface != i2.surface);
  }
  Shared_node(Info i1, Info i2, Info i3) : info{{i1, i2, i3}} {
    assert(i1.surface != i2.surface && i1.surface != i3.surface &&
           i2.surface != i3.surface);
  }
  Shared_node(Shared_node node, Info i)
      : info{{node.info[0], node.info[1], i}} {
    assert(node.degree() == 2);
    assert(i.surface != node.info[0].surface &&
  ...
  auto constraint() noexcept { return point().constraint; }
  auto is_valid() const {
  ...
  auto degree() const { return info[2].surface ? 3 : 2; }
};
