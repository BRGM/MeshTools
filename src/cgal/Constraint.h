...
struct Constraint {
  ...
  boost::variant<Relaxed_constraint, On_curve_sp, On_junction_sp> nature;
  operator bool() const {
  ...
  struct Collect_curves_visitor {
    ...
    Result operator()(const Relaxed_constraint&) const { return Result{}; }
  ...
  //        return Result{};
  //    }
  //    Result operator()(const On_junction_sp& sp1, const On_curve_sp& sp2)
  //    const {
  //        return this->operator()(sp2, sp1);
  //    }
  //    Result operator()(const On_junction_sp& sp1, const On_junction_sp& sp2)
  //    const {
  //        assert(sp1);
  ...
  struct Neighbors_on_same_curve_multivisitor {
    ...
    Result operator()(const On_junction_sp& coc1,
                      const On_junction_sp& coc2) const noexcept {
      assert(coc1 && coc2);
  ...
  bool has_curve(const Curve* pcurve) const {
    ...
    return std::find(begin(all_curves), end(all_curves), pcurve) !=
           end(all_curves);
  }
};

template <typename Surface>
inline auto neighbors_on_same_curve(const Constraint<Surface>& c1,
                                    const Constraint<Surface>& c2) {
