...
auto to_array(const Buffer& buffer) {
  ...
  return py::array_t<Scalar, py::array::c_style>{buffer.shape, buffer.stride,
                                                 buffer.data};
}
...
void add_c3t3_wrapper(py::module& module) {
  ...
  py::class_<C3t3Wrapper>(module, "C3t3")
      .def(py::init([](const std::string& filename, bool binary) {
             ...
             if (binary)
               wrapper->binary_reload(filename);
             else
      ...
      .def_property_readonly(
          "vertices",
          [](C3t3Wrapper& self) { return to_array(self.vertices_buffer()); })
      .def_property_readonly(
          "cells",
          [](C3t3Wrapper& self) { return to_array(self.cells_buffer()); })
      .def_property_readonly(
          "domain_tags",
          [](C3t3Wrapper& self) { return to_array(self.domain_tags_buffer()); })
      .def_property_readonly(
          "facets",
          [](C3t3Wrapper& self) { return to_array(self.facets_buffer()); })
      .def_property_readonly(
          "facet_tags",
          [](C3t3Wrapper& self) { return to_array(self.facet_tags_buffer()); })
      .def("output_boundaries_to_off",
           [](C3t3Wrapper& self, typename C3t3Wrapper::Domain_tag subdomain,
              py::str filename) {
      ...
      .def(
          "collect_boundaries",
          [](C3t3Wrapper& self, typename C3t3Wrapper::Domain_tag subdomain,
             bool normals_point_outside) {
            Triangulated_surface mesh;
            copy_boundary_to_surface_mesh(self.get(), subdomain, mesh,
                                          normals_point_outside);
            return mesh;
          },
          py::arg("subdomain"), py::arg("normals_point_outside") = true)
      .def(
          "collect_all_boundaries",
          [](C3t3Wrapper& self, bool normals_point_outside) {
            return collect_all_boundaries_as_surface_meshes<
                Triangulated_surface>(self.get(), normals_point_outside);
          },
