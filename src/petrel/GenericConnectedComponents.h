...
class ConnectedComponents {
 ...
 public:
  ...
  ConnectedComponents(InputIterator first, InputIterator last,
                      const NeighborhoodFactory& neighborhood_factory)
      : components{} {
    ...
    auto next_component = [&connections](Component_id ci) {
    ...
    std::set<Component_id> components_ids{connections.cbegin(),
                                          connections.cend()};
    ...
    for (auto p = first; p != last; ++p) {
      ...
      components[component_final_id[connections[components_map[handle]]]]
          .push_back(handle);
    }
...
// auto convert_components(const Complex& complex, const
// ConnectedComponents<Complex>& components)
//{
//    typedef typename Complex::Cell_handle Cell_handle;
//    std::map<Cell_handle, CellId> cmap;
//    auto ci = CellId{ 0 };
//    for (auto cell = complex.cells_in_complex_begin(); cell !=
//    complex.cells_in_complex_end(); ++cell) {
