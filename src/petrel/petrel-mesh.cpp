...
struct Segment_intersection_report {
  ...
  Segment_intersection_report(Intersections_map& pts) : points{pts} {}
...
auto soup_intersection(Soup& soup1, Soup& soup2) {
  ...
  CGAL::box_intersection_all_pairs_d(begin(boxes1), end(boxes1), begin(boxes2),
                                     end(boxes2), report);
  return points;
}
...
auto split_segment(Input_iterator p, Input_iterator q) {
  ...
  std::transform(
      p, q, std::back_inserter(all_points),
      [](const typename Input_iterator::value_type& r) { return r.second; });
  all_points.emplace_back(S->target());
  auto less_along_S = [O, u](const Point& P, const Point& Q) {
    return u * (P - O) < u * (Q - O);
  };
...
struct Neighborhood {
  ...
  Neighborhood(CDT* p, CDT::Face_handle f) : cdt{p}, face{f} {
    assert(p != nullptr);
  }
...
struct NeighborhoodFactory {
  ...
  NeighborhoodFactory(CDT* p) : cdt{p} { assert(cdt != nullptr); }
  auto operator()(CDT::Face_handle f) const { return Neighborhood{cdt, f}; }
};

template <typename SegmentSoup>
auto mesh_segment_soup(const SegmentSoup& soup) {
  CDT cdt;
  for (auto&& S : soup) cdt.insert_constraint(S.source(), S.target());

  const std::size_t nv = cdt.number_of_vertices();
  auto vertices = py::array_t<double, py::array::c_style>{
      {nv, static_cast<std::size_t>(2)}};
  ...
  typedef ConnectedComponents<NeighborhoodFactory::Element_handle>
      Connected_components;
  auto components =
      Connected_components{cdt.finite_faces_begin(), cdt.finite_faces_end(),
                           NeighborhoodFactory{&cdt}};
...
auto split_and_mesh(py::array_t<double, py::array::c_style> vertices,
  ...
  auto nodes =
      reinterpret_cast<const Point*>(vertices.unchecked<2>().data(0, 0));
  ...
  typedef CGAL::Box_intersection_d::Box_with_handle_d<
      CGAL::Epick::FT, Point::Ambient_dimension::value,
      SegmentSoup::const_iterator>
      SBox;
  ...
  auto add_non_splitted_segments =
      [&intersections, &splitted_segments](const SegmentSoup& soup) {
        for (auto p = cbegin(soup); p != cend(soup); ++p)
          if (intersections.erase(p) == 0) splitted_segments.emplace_back(*p);
      };
...
auto build_constrained_delaunay_triangulation(
  ...
  return build_constrained_delaunay_triangulation(
      reinterpret_cast<const Point*>(vertices.unchecked<2>().data(0, 0)),
      segments.unchecked<2>().data(0, 0), segments.shape(0));
}

auto mesh(py::array_t<double, py::array::c_style> segment_vertices,
          py::array_t<int, py::array::c_style> segments) {
  // py::print(segment_vertices);
  // py::print(segments);
  auto cdt =
      build_constrained_delaunay_triangulation(segment_vertices, segments);
  ...
  for (auto&& component : components) {
    ...
    while (!constrained_edges.empty()) {
      for (edge = begin(constrained_edges); edge != end(constrained_edges);
           ++edge) {
...
PYBIND11_MODULE(PetrelMesh, module) {
  ...
  py::class_<CDT>(module, "CDT").def(py::init([](std::string filename) {
  ...
  module.def("connected_components", [](CDT& cdt) {
    return Connected_components{cdt.finite_faces_begin(),
