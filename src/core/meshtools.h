...
//        this is for compatibility reasons with ComPASS internal Fortran mesh
//        structures
...
struct Element_by_nodes {
  ...
  // CHECKME: We fix the first constructor parameter not to shadow other
  // constructor (copy constructor...)
  //          Otherwise variant compilation fails with gcc
  template <typename... NodeIds>
  Element_by_nodes(NodeId n0, NodeIds... nis) : nodes{{n0, nis...}} {
    static_assert(sizeof...(NodeIds) == nn - 1, "Wrong nomber of nodes!");
    static_assert(is_single_type<std::tuple<NodeIds...>>(),
                  "Wrong type for nodes ids!");
    static_assert(
        std::is_same<NodeId, typename IsSingleTypeTuple<std::tuple<
                                 NodeIds...>>::First_element_type>::value,
        "Wrong type for nodes ids!");
  }
  Element_by_nodes() : nodes{} {}
...
struct With_facets {
  ...
  typedef std::array<Facets_type, std::tuple_size<Facet_types>::value>
      Facets_array;
  constexpr static std::size_t nbfacets() {
    return std::tuple_size<Facet_types>::value;
  }
};

struct Line : Element_by_nodes<2> {
  using Element_by_nodes<nbnodes()>::Element_by_nodes;
  static constexpr auto name = "Line";
  static constexpr auto VTK_ID = VTK_ID_type{3};
};

struct Triangle : Element_by_nodes<3>, With_facets<Line, Line, Line> {
  using Element_by_nodes<nbnodes()>::Element_by_nodes;
  static constexpr auto name = "Triangle";
  static constexpr auto VTK_ID = VTK_ID_type{5};
  auto facets() const -> Facets_array {
    return Facets_array{Line{nodes[0], nodes[1]}, Line{nodes[1], nodes[2]},
                        Line{nodes[2], nodes[0]}};
  };
};

struct Quad : Element_by_nodes<4> {
  using Element_by_nodes<nbnodes()>::Element_by_nodes;
  static constexpr auto name = "Quad";
  static constexpr auto VTK_ID = VTK_ID_type{9};
};

struct Tetrahedron : Element_by_nodes<4>,
  ...
  auto facets() const -> Facets_array {
    return Facets_array{Triangle{nodes[1], nodes[2], nodes[3]},
...
struct Hexahedron : Element_by_nodes<8>,
  ...
  // Outside oriented facets
  auto facets() const -> Facets_array {
    return Facets_array{Quad{nodes[0], nodes[1], nodes[2], nodes[3]},
...
struct Wedge : Element_by_nodes<6>,
  ...
  // Outside oriented facets
  auto facets() const -> Facets_array {
    return Facets_array{Triangle{nodes[0], nodes[1], nodes[2]},
...
struct Pyramid : Element_by_nodes<5>,
...
constexpr auto change_element_nodes(
    const mapbox::util::variant<Alternatives...>& variant,
    const std::map<NodeId, NodeId>& new_nodes) {
  typedef mapbox::util::variant<Alternatives...> Element;
  return mapbox::util::apply_visitor(
      [&new_nodes](auto alternative) {
        return Element{change_element_nodes(alternative, new_nodes)};
      },
      variant);
}
...
struct ArrayBaseAccessor {
  ...
  template <typename T, std::size_t n>
  static constexpr T extract(const std::array<T, n>& a,
...
// FIXME/IMPROVE: This is necessary only because Element_by_nodes does not
// inherit from std::array<NodeId, n>
struct NodesBaseAccessor {
  ...
  template <typename Element>
  static constexpr NodeId extract(const Element& element,
                                  std::size_t k) noexcept {
    assert(k < Element::nbnodes());
    return element.nodes[k];
  }
...
struct VariantAccessor : BaseAccessor {
  ...
  static constexpr auto size(
      const mapbox::util::variant<Alternatives...>& variant) noexcept {
    return mapbox::util::apply_visitor(
        [](auto alternative) { return BaseAccessor::size(alternative); },
        variant);
  }
  ...
  static constexpr auto begin(
      const mapbox::util::variant<Alternatives...>& variant) noexcept {
    // we need reference here because we return a pointer and we don't want to
    // make a copy of element
    return mapbox::util::apply_visitor(
        [](const auto& alternative) {
          return BaseAccessor::begin(alternative);
        },
        variant);
  }
  using BaseAccessor::access;
  template <typename... Alternatives>
  static constexpr auto access(mapbox::util::variant<Alternatives...>& variant,
                               std::size_t k) noexcept {
    return mapbox::util::apply_visitor(
        [k](auto& alternative) { return BaseAccessor::access(alternative, k); },
        variant);
  }
  ...
  static constexpr auto extract(
      ...
      std::size_t k) noexcept {
    return mapbox::util::apply_visitor(
        [k](const auto& alternative) {
          return BaseAccessor::extract(alternative, k);
        },
        variant);
  }
};
...
struct Range {
 ...
 public:
  Range(access_type first, access_type last) : bounds{first, last} {}
  Range(access_type&& first, access_type&& last)
      : bounds{std::forward<access_type>(first),
               std::forward<access_type>(last)} {}
...
template <typename... Alternatives>
inline constexpr VTK_ID_type vtk_id(
    const mapbox::util::variant<Alternatives...>& variant) noexcept {
  return mapbox::util::apply_visitor(
      [](const auto& alternative) { return vtk_id(alternative); }, variant);
}

template <typename Accessor, typename IDType,
          typename PointerType = DefaultCOCPointerType, typename Collection>
...
template <typename IDType = NodeId,
          typename PointerType = DefaultCOCPointerType, typename Collection>
inline auto node_collection_as_COC_data(const Collection& collection) {
  return collection_as_COC_data<VariantAccessor<NodesBaseAccessor>, IDType,
                                PointerType>(collection);
}
...
struct RawFaceIndex {
 ...
 protected:
  Index index;
  /** A rotation is performed on the nodes number so that the smallest is the
   * first. */
  void sort_nodes_for_index() {
    static_assert(std::tuple_size<Index>::value,
                  "Faces must have 2 nodes at least.");
 ...
 public:
  RawFaceIndex() = delete;
  RawFaceIndex(const RawFacetType& facet) : index{facet.nodes} {
    sort_nodes_for_index();
  }
  auto as_facet() const { return Facet_type{index}; }
...
using AllRawFacetTypes =
    typename ConcatenatedTupleSet<typename CellTypes::Facet_types...>::type;
...
template <typename... CellTypes>
using FaceIndex = VariantFromTupleIfNeeded<
    typename RawFaceIndexTuple<AllRawFacetTypes<CellTypes...>>::type>;
...
class FaceNeighbors {
 ...
 public:
  ...
  FaceNeighbors(const CellId& cell) : neighbors{cell, cell} {}
  bool is_on_boundary() const { return neighbors.first == neighbors.second; }
  bool is_inside() const { return neighbors.first != neighbors.second; }
...
template <typename IDType = FaceId,
          typename PointerType = DefaultCOCPointerType, typename Collection>
inline auto face_collection_as_COC_data(const Collection& collection) {
  return collection_as_COC_data<VariantAccessor<ArrayBaseAccessor>, IDType,
                                PointerType>(collection);
}
...
struct AllCellFacesId<CellType, CellTypes...> {
  ...
  typedef typename ExtendTupleSet<
      Cell_faces_id_subtuple, First_element_type,
      !IsInTuple<First_element_type,
                 Cell_faces_id_subtuple>::value>::Extended_tuple_type type;
};
...
struct AllNewFacetArray<NewFacetType, CellType, CellTypes...> {
  typedef typename AllNewFacetArray<NewFacetType, CellTypes...>::type
      New_facet_array_subtuple;
  typedef ::std::array<NewFacetType, CellType::nbfacets()> First_element_type;
  typedef typename ExtendTupleSet<
      New_facet_array_subtuple, First_element_type,
      !IsInTuple<First_element_type,
                 New_facet_array_subtuple>::value>::Extended_tuple_type type;
};
...
// template <typename NewFacetType, typename ... CellTypes>
// using MeshFacetArray = VariantFromTupleIfNeeded<typename
// AllNewFacetArray<NewFacetType, CellTypes...>::type>;
...
struct MeshTraits {
  ...
  typedef VariantFromTupleIfNeeded<typename AllCellFacesId<CellTypes...>::type>
      Cell_faces_id;
  typedef VariantFromTupleIfNeeded<AllRawFacetTypes<CellTypes...>> Facet_type;
  typedef VariantFromTupleIfNeeded<
      typename AllNewFacetArray<Facet_type, CellTypes...>::type>
      Facets_array;
...
struct MeshConnectivity {
  ...
  static auto recast_facet(const mapbox::util::variant<FacetTypes...>& vfacet) {
    typedef typename Mesh_traits::Facet_type Facet_type;
    return mapbox::util::apply_visitor(
        [](const auto& facet) { return Facet_type{facet}; }, vfacet);
  }
  template <typename FacetType>
  ...
  template <typename... FacetTypes>
  static auto make_face_index(
      const mapbox::util::variant<FacetTypes...>& vfacet) {
    typedef typename Mesh_traits::Face_index Face_index;
    return mapbox::util::apply_visitor(
        [](const auto& facet) { return Face_index{facet}; }, vfacet);
  }
  ...
  static auto extract_cell_facets(const CellType& cell) {
    ...
    result.reserve(n);
    // Loop shall be unrolled as n is small and known at compile time
    // (constexpr)
  ...
  static auto extract_cell_facets(
      const mapbox::util::variant<CellTs...>& vcell) {
    return mapbox::util::apply_visitor(
        [](const auto& cell) { return extract_cell_facets(cell); }, vcell);
  }
  struct Cells {
    typedef typename Mesh_traits::Cell_type Cell_type;
    ...
    Nodes nodes;
    Faces faces;
    auto nb() const { return nodes.size(); }
  };
  struct Faces {
    ...
    static auto extract_facet_from_index(
        const mapbox::util::variant<FaceIndexTypes...>& vindex) {
      return mapbox::util::apply_visitor(
          [](const auto& index) { return extract_facet_from_index(index); },
          vindex);
    }
    // template <std::size_t n>
    // void register_facets(const CellId& cellid, const std::array<Facet_type,
    // n>& facets) {
    //}
    // template <typename ... FacetsArray>
    // void register_facets(const CellId& cellid, const
    // mapbox::util::variant<FacetsArray...>& vfacets) {
    //	mapbox::util::apply_visitor([this, &cellid](const auto& facets) {
    // register_facets(cellid, facets); }, vfacets);
    //}
  ...
  auto collect_cell_faces_id(mapbox::util::variant<CellTs...>& vcell) const {
    return mapbox::util::apply_visitor(
        [this](const auto& cell) { return this->collect_cell_faces_id(cell); },
        vcell);
  }
  ...
  void remap_cellnodes(const Cell_type* first, const Cell_type* last,
                       std::map<NodeId, NodeId>& new_nodes) {
    cells.nodes.clear();
    cells.faces.clear();
    cells.nodes.reserve(std::distance(first, last));
    std::transform(first, last, std::back_inserter(cells.nodes),
...
struct Point : ::std::array<Coordinate, dim> {
  ...
  Point(Coordinate_type x0, Coordinates... xis)
    ...
    static_assert(sizeof...(Coordinates) == dim - 1,
                  "Wrong nomber of coordinates!");
    static_assert(is_single_type<std::tuple<Coordinates...>>(),
                  "Wrong type for nodes ids!");
    static_assert(
        std::is_same<Coordinate_type,
                     typename IsSingleTypeTuple<std::tuple<Coordinates...>>::
                         First_element_type>::value,
        "Wrong type for nodes ids!");
  }
  Point() : Base_array{} {}
  Point(const Base_array& a) : Base_array{a} {}
  Point(Base_array&& a) : Base_array{std::forward<Base_array>(a)} {}
...
struct PointAccumulator {
  ...
  std::size_t n;
  PointAccumulator() : accumulation{Point_type::origin()}, n{0} {}
  PointAccumulator(const Point_type& P) : accumulation{P}, n{1} {}
  PointAccumulator(Point_type&& P)
      : accumulation{std::forward<Point_type>(P)}, n{1} {}
...
struct Mesh {
  ...
  void apply_on_vertices(
      Functor& F,
      const mapbox::util::variant<NodeElementTypes...>& velement) const {
    mapbox::util::apply_visitor(
        [this, &F](const auto& element) {
          this->apply_on_vertices(F, element);
        },
        velement);
  }
  ...
  auto element_vertices(const MeshElement& element) const {
    static_assert(
        IsInTuple<MeshElement, typename Mesh_traits::Cell_types>::value,
        "Mesh does not hold elements of this kind!");
