...
// cf. doc at
// http://pybind11.readthedocs.io/en/stable/advanced/cast/stl.html?highlight=variant#c-17-library-containers
namespace pybind11 {
namespace detail {
...
struct visit_helper<mapbox::util::variant> {
  ...
  static auto call(Args&&... args)
      -> decltype(mapbox::util::apply_visitor(args...)) {
...
namespace detail {
template <typename T>
class type_caster<std::vector<T>> : public type_caster_base<std::vector<T>> {};
}  // namespace detail
}  // namespace pybind11
...
inline auto build_from_array_like(py::array_t<T, py::array::c_style> a)
  ...
  return std::make_unique<TargetType>(
      *(reinterpret_cast<Array*>(a.mutable_data(0))));
}
...
void add_element(py::module& module) {
  py::class_<Element>(module, Element::name, py::buffer_protocol())
      ...
      // OPTIMIZE: As Elements have small footprint in memory that might be more
      // efficient to perform a direct copy
      .def_buffer([](Element& self) -> py::buffer_info {
        return py::buffer_info(self.nodes.data(), sizeof(MT::NodeId),
                               py::format_descriptor<MT::NodeId>::format(), 1,
                               {Element::nbnodes()}, {sizeof(MT::NodeId)});
      })
      .def("vtk_id", [](const Element&) { return Element::VTK_ID; })
...
auto extract_ids(const Collection& elements)
    -> py::array_t<MT::VTK_ID_type, py::array::c_style> {
...
// CHECKME: This version relies on std::map binary search to build part without
// knowing their number
//          when the number of part is known the other implementation might be
//          more efficient
...
template <typename ItemId, typename ProcId>
auto distribute_items(const std::size_t nbprocs, const std::size_t nbitems,
                      const ProcId* itemproc) {
...
// FIXME: This is to replace std::vector<T>(std::size_t count, const T& value)
// constructor
...
auto distribute(const Mesh& mesh,
                py::array_t<ProcId, py::array::c_style> cellproc) -> py::list {
  ...
  // FIXME/IMPROVE: things could be factorized here if node range was acces by
  // range and not node_range Process nodes
  ...
  auto own_nodes = distribute_items<MeshTools::NodeId>(nbprocs, nodeproc.size(),
                                                       nodeproc.data());
  ...
  auto own_faces = distribute_items<MeshTools::FaceId>(nbprocs, faceproc.size(),
                                                       faceproc.data());
...
auto locate_all_cell_faces(
    const Mesh& mesh,
    py::array_t<MeshTools::CellId, py::array::c_style> cells) {
...
//          for "decltype workaround" to work for MSVC
//          cf. line with
//          (decltype(&locate_cell_faces<Mesh>))&locate_cell_faces<Mesh> there
//          "decltype workaround" itself is due to a gcc bug...
...
auto identify_faces_from_positions(
    const Mesh& mesh, py::array_t<MeshTools::CellId, py::array::c_style> cell,
  ...
  for (std::size_t k = 0; k != nf; ++k) {
    face_id(k) = Accessor::extract(mesh.connectivity.cells.faces[face_cell(k)],
                                   face_position(k));
  }
  return result;
}
...
//          "decltype workaround" itself is due to a gcc bug...
template <typename Mesh>
auto locate_faces_with_cell(
    const Mesh& mesh, py::array_t<MeshTools::CellId, py::array::c_style> cells,
    py::array_t<MeshTools::FaceId, py::array::c_style> faces) -> py::tuple {
...
// CHECKME: output type declaration (i.e. -> py::array_t<...>) is necessary in
// MSVC
//          for "decltype workaround" to work below
//          "decltype workaround" itself is due to a gcc bug...
template <typename Vector>
auto as_raw_array(const Vector& v) -> pyMT::RawArray {
  ...
  return pyMT::RawArray{{v.size(), sizeof(value_type)},      // shape
...
void set_mesh_vertices(
  ...
  static_assert(sizeof(Vertex) == dim * sizeof(Coordinate),
...
auto add_mesh(py::module module) {
  ...
  // The following would give acess to buffer view of CellNodes vector but
  // desactivate variant caster py::class_<typename Cells::Cell_type>(module,
  // "Cell_type", py::module_local());
  ...
  // CHECKME: Without the CheckBinds counter the following bind_vector would
  // fail at import with
  //          ImportError: generic_type: type "FacesNodes" is already
  //          registered!
          ...
          [](const Faces& self) {
            return py::array_t<MeshTools::CellId, py::array::c_style>{
                ...
                std::vector<std::size_t>{
                    {2 * sizeof(MeshTools::CellId),
                     sizeof(MeshTools::CellId)}},  // strides
                reinterpret_cast<const MeshTools::CellId*>(self.cells.data())};
          },
          py::keep_alive<0, 1>())
      .def("id", [](const Faces& self,
                    const typename Mesh_traits::Facet_type& facet) {
      ...
      py::class_<Mesh>(module, "Mesh", py::module_local())
          ...
          .def(
              ...
              [](const Mesh& self) {
                ...
                return py::array_t<Coordinate_type, py::array::c_style>{
                    std::vector<std::size_t>{
                        {self.vertices.size(), Mesh::dimension}},  // shape
                    std::vector<std::size_t>{
                        {Mesh::dimension * sizeof(Coordinate_type),
                         sizeof(Coordinate_type)}},  // strides
                    reinterpret_cast<const Coordinate_type*>(
                        self.vertices.data())};
              },
              py::keep_alive<0, 1>())
          .def_property_readonly(
              "nb_vertices",
              [](const Mesh& mesh) { return mesh.vertices.size(); })
          .def_property_readonly(
              "nb_cells",
              [](const Mesh& mesh) { return mesh.connectivity.cells.nb(); })
          .def_property_readonly(
              "nb_faces",
              [](const Mesh& mesh) { return mesh.connectivity.faces.nb(); })
          .def("cells_nodes_as_COC",
               [](const Mesh& mesh) {
                 return MT::node_collection_as_COC_data(
                     mesh.connectivity.cells.nodes);
               })
          .def("faces_nodes_as_COC",
               [](const Mesh& mesh) {
                 return MT::node_collection_as_COC_data(
                     mesh.connectivity.faces.nodes);
               })
          .def("cells_faces_as_COC",
               [](const Mesh& mesh) {
                 return MT::face_collection_as_COC_data(
                     mesh.connectivity.cells.faces);
               })
          .def("COC_data",
               [](const Mesh& mesh) {
                 return py::make_tuple(MT::node_collection_as_COC_data(
                                           mesh.connectivity.cells.nodes),
                                       MT::face_collection_as_COC_data(
                                           mesh.connectivity.cells.faces),
                                       MT::node_collection_as_COC_data(
                                           mesh.connectivity.faces.nodes));
               })
          .def("cells_vtk_ids",
               [](const Mesh& mesh) {
          ...
          .def(py::pickle(
              ...
              [](py::tuple t) {  // __setstate__
                ...
                auto mesh = Mesh{};
                mesh.vertices = from_raw_array<typename Mesh::Vertices>(
                    t[0].cast<pyMT::RawArray>());
                mesh.connectivity.cells.nodes =
                    from_raw_array<typename Cells::Nodes>(
                        t[1].cast<pyMT::RawArray>());
...
void set_uniform_mesh_cellnodes(
  ...
  static_assert(sizeof(Cell_type) == nn * sizeof(MT::NodeId),
                "Inconsistent sizes in memory.");
...
struct CellTypeAttempt<MeshType, N, true> {
  ...
  static Cell_type cast(py::object object) {
    ...
    try {
      ...
      return CellTypeAttempt<
          MeshType, N + 1,
          N + 1 < std::tuple_size<Cell_types>::value>::cast(object);
    }
    assert(false);  // never reached
    return Cell_type{};
  }
...
template <typename MeshType>
inline auto cast_as_celltype(py::object object) ->
    typename MeshType::Mesh_traits::Cell_type {
  typedef typename MeshType::Mesh_traits::Cell_types Cell_types;
  static_assert(std::tuple_size<Cell_types>::value > 0, "Empty tuple!");
  return CellTypeAttempt<MeshType, 0,
                         0 < std::tuple_size<Cell_types>::value>::cast(object);
}
...
//          for "decltype workaround" to work in add_uniform_mesh_submodule for
//          MSVC (cf. line with
//          (decltype(&make_uniform_mesh<Mesh_type>))&make_uniform_mesh<Mesh_type>
//          there) "decltype workaround" itself is due to a gcc bug...
template <typename MeshType>
...
//          for "decltype workaround" to work in add_uniform_mesh_submodule for
//          MSVC (cf. line with
//          (decltype(&rebuild_mesh<Mesh_type>))&rebuild_mesh<Mesh_type> there)
//          "decltype workaround" itself is due to a gcc bug...
template <typename MeshType, typename byte = int8_t>
auto create_from_remap(
    py::object vertices, py::array_t<byte, py::array::c_style> raw_cellnodes,
...
auto add_uniform_mesh_submodule(py::module& module, const std::string& name) {
  ...
  // FIXME: TO be replaced by structured bindings in C++17 (e.g: auto
  // [submodule, mesh_class] = add_mesh_submodule...)
  ...
  mesh_class.def("face_id", [](const Mesh_type& self,
    ...
    if (position == faces_id_map.end())
      return static_cast<MT::FaceId>(self.nb_faces());
...
void add_mesh_tools(py::module& module) {
  ...
  py::class_<Point>(module, "Point")
      .def(py::init(
          [](py::array_t<typename Point::Coordinate_type, py::array::c_style>
                 a) {
  ...
  // bind_vector_with_array_view<std::vector<std::size_t>>(module,
  // "COCPointersVector");
  ...
  // FIXME: The following would require to define 2D elements facets (as 3D
  // objects or 1D ?)
  // add_mesh_submodule<Mesh<MT::Triangle, MT::Quad, MT::Tetrahedron, MT::Wedge,
  // MT::Hexahedron>>(module, "HybridDimensionMesh");
  // add_mesh_submodule<Mesh<MT::Tetrahedron, MT::Wedge,
  // MT::Hexahedron>>(module, "HybridMesh");
  add_mesh_submodule<
      Mesh<MT::Tetrahedron, MT::Wedge, MT::Pyramid, MT::Hexahedron>>(
      module, "HybridMesh");
