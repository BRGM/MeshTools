...
template <typename TupleType>
using VariantFromTupleSet =
    typename VariantFromTuple<typename TupleSet<TupleType>::type>::type;
...
struct MakeVariantFromTuple<TupleType, true> {
  static_assert(!is_single_type<TupleType>(),
                "Tuple is single type, don't use variant!");
...
template <typename TupleType>
using VariantFromTupleIfNeeded =
    typename MakeVariantFromTuple<TupleType,
                                  !is_single_type<TupleType>()>::type;
