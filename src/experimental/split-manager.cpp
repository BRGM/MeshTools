...
struct Faces_map {
    ...
    void _collect_edges(const IdType face_id, py::sequence nodes) {
        ...
        for (int k = 0; k < n; ++k) {
            const auto e = edge_type{
                nodes[k - 1].cast<IdType>(),
                nodes[k].cast<IdType>()
            };
...
public:
    Splittables(const std::size_t n) :
        elements{ n } {
...
PYBIND11_MODULE(SplitManager, module)
{
    ...
    module.doc() = "pybind11 split manager to help managing splitting elements (quick and dirty!!!)";
    ...
    py::class_<pyEdge>(module, "Edge")
        ...
        if (py::len(seq) != 2) throw std::runtime_error("edge only have two vertices");
        return std::make_unique<pyEdge>(
            seq[0].cast<Id_type>(), seq[1].cast<Id_type>()
            );
    }))
        .def("__lt__", [](const pyEdge& self, const pyEdge& other) {
        return self < other;
    })
    ...
    py::class_<pyFaces_map>(module, "Faces_map")
        ...
        .def("faces", [](const pyFaces_map& self) {
        return py::make_iterator(
            begin(self.face_nodes), end(self.face_nodes)
            );
        })
