...
struct Handle {
  ...
  Handle(const Handle_type& h, const Ts&... is) : handle{h}, indices{{is...}} {
...
auto add_edge_constraints_map(Surface_mesh& mesh) {
  ...
  auto pmap = mesh.template add_property_map<Edge_index, bool>(
...
auto get_edge_constraints_map(Surface_mesh& mesh) {
  typedef typename Surface_mesh::Edge_index Edge_index;
  auto pmap =
      mesh.template property_map<Edge_index, bool>(edge_constraints_map_name);
  assert(pmap.second);
  return pmap.first;
};

template <typename Triangulation2, typename Surface_mesh, typename Converter>
auto triangulation_to_surface_mesh_vmap(const Triangulation2& triangulation,
                                        Surface_mesh& mesh,
                                        const Converter& convert) {
  std::map<typename Triangulation2::Vertex_handle,
           typename Surface_mesh::Vertex_index>
      vmap;
...
void triangulation_to_surface_mesh_add_constraints(
    const Triangulation2& triangulation, const VMap& vmap, Surface_mesh& mesh) {
  auto is_constrained_edge = get_edge_constraints_map(mesh);
  for (auto ce = triangulation.constrained_edges_begin();
       ce != triangulation.constrained_edges_end(); ++ce) {
...
template <typename Triangulation2, typename VMap, typename Surface_mesh>
void triangulation_to_surface_mesh_add_faces(
    const Triangulation2& triangulation, const VMap& vmap, Surface_mesh& mesh) {
  for (auto f = triangulation.finite_faces_begin();
       f != triangulation.finite_faces_end(); ++f) {
    mesh.add_face(vmap.at(f->vertex(0)), vmap.at(f->vertex(1)),
                  vmap.at(f->vertex(2)));
  }
}

template <typename Triangulation2, typename Surface_mesh, typename Converter>
void triangulation_to_surface_mesh(const Triangulation2& triangulation,
                                   Surface_mesh& mesh,
                                   const Converter& convert) {
  mesh.clear();
...
template <typename Triangulation2, typename Surface_mesh, typename Converter>
void triangulation_to_surface_mesh_with_constraints(
    const Triangulation2& triangulation, Surface_mesh& mesh,
    const Converter& convert) {
  mesh.clear();
  auto vmap = triangulation_to_surface_mesh_vmap(triangulation, mesh, convert);
  triangulation_to_surface_mesh_add_faces(triangulation, vmap, mesh);
...
void triangulation_to_surface_mesh_with_constraints(
    const Triangulation2& triangulation, Surface_mesh& mesh) {
  triangulation_to_surface_mesh_with_constraints(triangulation, mesh,
                                                 Default_converter{});
}
...
void elevation_surface(const PointRange& points, Surface_mesh& mesh) {
  ...
  auto to_surface_kernel =
      CGAL::Cartesian_converter<Point_kernel, Surface_kernel>{};
...
struct URGMesh {
  ...
  URGMesh(const Hull& hull, const Definitions& definitions) {
    ...
    const auto j = hS[hj].indices[0] == i ? hS[hj].indices[1] :
    hS[hj].indices[0]; const auto k = hS[hk].indices[0] == i ? hS[hk].indices[1]
    : hS[hk].indices[0]; if (i < j && j < k) { auto I = CGAL::intersection(Sj,
    Sk); if (I) { if (const Point *P = boost::get<Point>(&*I)) { auto hCijk =
    Corner_handle{ corners.size(), i, j, k }; corners.emplace_back(*P);
    ...
    */
    ...
    typedef CGAL::Constrained_Delaunay_triangulation_2<Tk, CGAL::Default, Itag>
        CDT;
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      for (auto&& e : hull_mesh.edges()) {
        if (hull_mesh.is_border(e)) {
          auto border_edge =
              domain[i].clip(Segment{hull_mesh.point(hull_mesh.vertex(e, 0)),
                                     hull_mesh.point(hull_mesh.vertex(e, 1))});
          if (border_edge) {
            // std::cerr << "clipped border edge " << i << std::endl;
            cdt.insert_constraint(to_Tk(Pi.to_2d(border_edge->source())),
      ...
      for (auto&& Sh : segment_handles[i]) {
        ...
        cdt.insert_constraint(to_Tk(Pi.to_2d(S.source())),
      ...
      auto aPi = to_Tk(Pi);
      triangulation_to_surface_mesh_with_constraints(
          cdt, mesh, [&aPi](const auto& P) { return aPi.to_3d(P); });
    }
  }
  ...
  void add_surface(const Surface_mesh& new_mesh) {
    ...
    for (auto&& S : surface_meshes) {
      ...
      CGAL::Polygon_mesh_processing::corefine(
          S, mesh, parameters::edge_is_constrained_map(emap),
...
auto make_mesh(const Hull& hull, py::list& l) {
  ...
  std::vector<
      std::pair<typename Hull_kernel::Point_3, typename Hull_kernel::Vector_3> >
      definitions;
  for (auto&& p : l) {
    auto t = p.cast<py::tuple>();
    definitions.emplace_back(to_hull_kernel(t[0].cast<Point>()),
...
PYBIND11_MODULE(URG, module) {
  ...
  py::class_<Mesh>(module, "Mesh")
      ...
      .def(
          "surfaces",
          [](const Mesh& self) {
            return py::make_iterator(begin(self.surface_meshes),
                                     end(self.surface_meshes));
          },
          py::keep_alive<0, 1>())
      .def("isotropic_remeshing", &Mesh::isotropic_remeshing)
      .def("add_surface", &Mesh::add_surface);

  module.def("elevation_surface",
