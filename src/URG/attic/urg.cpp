...
struct Surface_vertex_index {
  // There would be much less lines of code direcly replacing the variant by a
  ...
  //        if (added < sia[0]) return std::array<Surface_index, 3>{ { added,
  //        sia[0], sia[1] } }; if (added < sia[1]) return
  //        std::array<Surface_index, 3>{ { sia[0], added, sia[1] } }; return
  //        std::array<Surface_index, 3>{ { sia[0], sia[1], added } };
  //    }
  //    result_type operator()(const std::array<Surface_index, 3>& sia) const {
  //        if (added == sia[0] || added == sia[1] || added == sia[2]) return
  //        sia; auto s = std::set<Surface_index>{ begin(sia), end(sia) };
  //        s.insert(added);
  //        return s;
  //    }
  ...
  //        return 1;
  //    }
  //    constexpr result_type operator()(const std::array<Surface_index, 2>&)
  //    const noexcept {
  //        return 2;
  //    }
  //    constexpr result_type operator()(const std::array<Surface_index, 3>&)
  //    const noexcept {
  //        return 3;
  //    }
  ...
  Surface_vertex_index(Surface_index si) : index{si} {}
  Surface_vertex_index& add(Surface_index si) {
    index.insert(si);
    return *this;
  }
  ...
  auto degree() const { return index.size(); }
};

template <typename Surface_index>
bool operator==(const Surface_vertex_index<Surface_index>& svi1,
                const Surface_vertex_index<Surface_index>& svi2) {
...
//    auto pmap = mesh.add_property_map<Vertex_index,
//    Vertex_degree>("vertex_degree", Vertex_degree{}); assert(pmap.second);
//    return pmap.first;
//}
//
...
//    auto pmap = mesh.add_property_map<Edge_index, bool>("is_constrained_edge",
//    false); assert(pmap.second); return pmap.first;
//}
...
struct Split_recorder {
  ...
  static void record_split(Mesh_type& mesh, Halfedge_index hold,
                           Halfedge_index hnew) {
    assert(mesh.next(hnew) == hold);
    assert(records_map.count(&mesh) == 1);
    records_map[&mesh].emplace_back(hold, hnew);
  }
  auto associate_records() { assert(records_map.size() == 2); }
};
...
namespace Euler {
template <typename Point>
typename boost::graph_traits<MyMesh<Point>>::halfedge_descriptor split_edge(
    typename boost::graph_traits<MyMesh<Point>>::halfedge_descriptor h,
    MyMesh<Point>& g) {
  auto hnew =
      opposite(CGAL::Euler::split_vertex(prev(h, g), opposite(h, g), g), g);
...
struct Surface_factory {
  ...
  struct Surface {
    ...
    struct Border_constraint {
      ...
      value_type id;
      constexpr bool operator<(const Border_constraint& other) const noexcept {
        return id < other.id;
      }
      constexpr bool operator==(const Border_constraint& other) const noexcept {
        return id == other.id;
      ...
      static constexpr value_type default_value() noexcept {
        return std::numeric_limits<Surface_index>::max();
      }
      Border_constraint() : id{default_value()} {}
      Border_constraint(const Border_constraint&) = default;
      Border_constraint& operator=(const Border_constraint&) = default;
      Border_constraint(value_type s) : id{s} {}
    ...
    constexpr static const auto edge_constraint_source_name =
        "e:constraint_source";
    Surface_mesh mesh;
    Surface_index index;
    ...
    void create_named_property_map(const char* name,
                                   value_type default_value = value_type{}) {
      auto pmap =
          mesh.add_property_map<Location, value_type>(name, default_value);
      assert(pmap.second);
    }
    template <typename Location, typename value_type>
    auto retrieve_named_property_map(const char* name) {
      auto pmap = mesh.property_map<Location, value_type>(name);
      assert(pmap.second);
      return pmap.first;
    }
    Surface(Surface_index si) : mesh{}, index{si}, external_constraints_map{} {
      // create_named_property_map<Vertex_index,
      // Vertex_info>(vertex_info_map_name, { index });
      create_named_property_map<Vertex_index, bool>(corners_map_name, false);
      create_named_property_map<Edge_index, bool>(edge_constraints_map_name,
                                                  false);
      create_named_property_map<Edge_index, Constraint>(
          edge_constraint_source_name);
    }
    // auto vertex_info_map() {
    //    return retrieve_named_property_map<Vertex_index,
    //    Vertex_info>(vertex_info_map_name);
    //}
    auto corners_map() {
      return retrieve_named_property_map<Vertex_index, bool>(corners_map_name);
    }
    auto edge_constraints_map() {
      return retrieve_named_property_map<Edge_index, bool>(
          edge_constraints_map_name);
    }
    auto edge_constraint_source_map() {
      return retrieve_named_property_map<Edge_index, Constraint>(
          edge_constraint_source_name);
    }
    template <typename Location, typename value_type>
    ...
    void register_external_constraints(Constraint c,
    ...
    void relax_corner(Vertex_index v, Constraint C) {
      ...
      auto is_corner = corners_map();
      assert(is_corner[v] && corner_constraints.count(v) == 1 &&
             corner_constraints[v].count(C) == 1);
      auto& s = corner_constraints[v];
      s.erase(C);
      if (s.size() < 3) {
    ...
    static void register_corner(Vertex_index v1, Constraint C1, Vertex_index v2,
                                Constraint C2, Vertex_index v3, Constraint C3) {
  ...
  Surface_factory() : new_index{0} {}
  Surface_factory(const Surface_factory&) = delete;
  Surface_factory& operator=(const Surface_factory&) = delete;
  auto make() { return Surface{new_index++}; }
  auto make_unique_pointer() { return std::make_unique<Surface>(new_index++); }
};
...
struct Boundary_hull {
  ...
  inline bool is_inside(const Point& P) const noexcept {
    return std::all_of(begin(planes), end(planes), [P](const auto& plane) {
  ...
  Boundary_hull(Plane_iterator first, Plane_iterator last)
      : planes{}, edges{}, corners{}, boundaries{} {
    typedef
        typename std::iterator_traits<Plane_iterator>::value_type Source_plane;
    ...
    for (Plane_index i = 0; i < n; ++i) {
      for (Plane_index j = i + 1; j < n; ++j) {
        for (Plane_index k = j + 1; k < n; ++k) {
          auto intersection =
              CGAL::intersection(planes[i], planes[j], planes[k]);
          if (intersection) {
    ...
    for (Plane_index i = 0; i < n; ++i) {
      ...
      std::cout << "The convex hull contains " << boundary.number_of_vertices()
                << " vertices and " << boundary.number_of_faces() << " faces"
                << std::endl;
      for (const auto& face : boundary.faces()) {
        std::cerr << "Face with " << boundary.degree(face) << " points"
                  << std::endl;
      }
    }
  ...
  void intersect(const Plane& plane, Surface_mesh& mesh) const {
    ...
    std::cerr << "Hull intersection with " << points.size() << " points."
              << std::endl;
...
auto constrained_midpoints(Surface& surface) {
  ...
  for (auto&& e : mesh.edges()) {
    if (is_constrained_edge[e]) {
      midpoints.emplace_back(to_epick(CGAL::midpoint(
          mesh.point(mesh.vertex(e, 0)), mesh.point(mesh.vertex(e, 1)))));
    }
  }
  auto result = py::array_t<double, py::array::c_style>{
      {midpoints.size(),
       static_cast<std::size_t>(Point::Ambient_dimension::value)}};
  auto p = reinterpret_cast<Point*>(result.request().ptr);
  static_assert(
      sizeof(Point) == Point::Ambient_dimension::value * sizeof(double),
...
struct On_plane_side {
 ...
 public:
  On_plane_side(const Plane& P, const Point& A)
      : plane{P}, side{plane.oriented_side(A)} {}
  On_plane_side(Plane&& P, const Point& A)
      : plane{std::forward<Plane>(P)}, side{plane.oriented_side(A)} {}
...
template <typename Surface_mesh>
inline auto face_barycenter(const typename Surface_mesh::Halfedge_index h,
...
//                //for (int i = 0; i < 2; ++i) {
//                //    vertex_info[mesh.vertex(e,
//                i)].remove(constraint_source);
//                //}
//                is_constrained_edge[e] = false;
//                edge_constraint_source[e] =
//                std::numeric_limits<Surface_index>::max();
//            }
//            assert(!is_constrained_edge[e]);
//            assert(edge_constraint_source[e] ==
//            std::numeric_limits<Surface_index>::max());
//        }
//    }
//    //update_vertex_properties(surface);
//    assert(
//        std::all_of(begin(relaxed_edge_constraints),
//        end(relaxed_edge_constraints),
//            [&surface](auto si) {
...
//    std::copy(begin(relaxed_edge_constraints), end(relaxed_edge_constraints),
//    std::ostream_iterator<Surface_index>(std::cerr, " ")); std::cerr <<
//    std::endl; return relaxed_edge_constraints;
//}
...
auto select_connected_components(Surface& surface, const Mask& is_kept) {
  ...
  for (auto&& f : mesh.faces()) {
    ...
    assert(is_kept_component[C] ==
           is_kept(face_barycenter(mesh.halfedge(f), mesh)));
  }
  ...
  std::cerr << "Relaxed constraints on " << surface.index << " : ";
  for (auto&& c : relaxed_edge_constraints)
    std::cerr << Surface::human_readable(c) << " ";
  std::cerr << std::endl;
  ...
  CGAL::Polygon_mesh_processing::keep_connected_components(
      mesh, kept_components, components);
...
inline auto select_faces(Surface& surface, const Mask& is_kept) {
  ...
  //            std::cerr << "!!!!!!!!!!!!!!!!!!!! " << e << " on " <<
  //            surface.index << " is constrained by " << si << "(" <<
  //            mesh.is_removed(e) << mesh.is_border(e) << ")" << std::endl;
  //            good = false;
  //        }
  //    }
  //    assert(good);
      ...
      if (is_center) {
        assert(std::all_of(
            ...
            [&mesh, &is_constrained_edge, &edge_constraint_source](auto h) {
              const auto e = mesh.edge(h);
              return (!is_constrained_edge[e]) &&
                     Surface::unspecified(edge_constraint_source[e]);
            }));
...
void simplify_face_borders(Surface& surface) {
  ...
  for (auto&& v : mesh.vertices()) {
    ...
    // assertion failure means a corner has been missed or a center vertex has
    // not been removed
    if (!(borders_around_v == 2 && constraints_around_vertex.empty()) &&
        !(borders_around_v == 2 && constraints_around_vertex.size() == 2 &&
          constraints_around_vertex.count(
              *(constraints_around_vertex.begin())) == 2) &&
        !(borders_around_v == 0 && constraints_around_vertex.size() == 2 &&
          constraints_around_vertex.count(
              *(constraints_around_vertex.begin())) == 2)) {
      typedef typename Surface::Surface_mesh Surface_mesh;
      typedef typename Surface_mesh::Point Surface_point;
      typedef
          typename CGAL::Kernel_traits<Surface_point>::Kernel Surface_kernel;
      typedef CGAL::Epick::Point_3 Point;
      auto to_epick = CGAL::Cartesian_converter<Surface_kernel, CGAL::Epick>{};
      std::cerr << "Corner pb: " << to_epick(mesh.point(v)) << " "
                << borders_around_v << " " << constraints_around_vertex.size()
                << std::endl;
    }
    assert((borders_around_v == 2 && constraints_around_vertex.empty()) ||
           (borders_around_v == 2 && constraints_around_vertex.size() == 2 &&
            constraints_around_vertex.count(
                *(constraints_around_vertex.begin())) == 2) ||
           (borders_around_v == 0 && constraints_around_vertex.size() == 2 &&
...
auto corefine(Surface& S1, Surface& S2) {
  ...
  CGAL::Polygon_mesh_processing::corefine(
      S1.mesh, S2.mesh, parameters::edge_is_constrained_map(constraints1),
  ...
  std::cerr << std::endl;
  auto apply_constraints = [](Surface& S, const auto& edge_is_constrained,
                              Surface& other) {
    typedef typename Surface::Constraint Constraint;
    auto& mesh = S.mesh;
      ...
      if (edge_is_constrained[e]) {
        if (has_constraint[e]) {
          std::cerr << "Inconsistency on surface " << S.index
                    << " corefined with " << other.index << " edge " << e
                    << " already constrained by "
                    << Surface::human_readable(constraint_source[e])
                    << std::endl;
          typedef typename Surface::Surface_mesh Surface_mesh;
          typedef typename Surface_mesh::Point Surface_point;
          typedef typename CGAL::Kernel_traits<Surface_point>::Kernel
              Surface_kernel;
          typedef CGAL::Epick::Point_3 Point;
          auto to_epick =
              CGAL::Cartesian_converter<Surface_kernel, CGAL::Epick>{};
    ...
    for (auto&& v : constrained_vertices) {
      if (!is_corner[v]) {
        ...
        if (!(vertex_constraints.size() == 2 &&
              vertex_constraints.count(*(vertex_constraints.begin())) == 2)) {
          is_corner[v] = true;
        }
  ...
  const auto n =
      std::count(std::begin(constraints1), std::end(constraints1), true);
  assert(std::count(std::begin(constraints2), std::end(constraints2), true) ==
         n);
...
Factory& get_surface_factory() { return surface_factory; }

PYBIND11_MODULE(URG, module) {
  module.doc() = "plane intersection for URG model";

  py::class_<Point>(module, "Point").def(py::init<double, double, double>());

  py::class_<Vector>(module, "Vector").def(py::init<double, double, double>());
  ...
  py::class_<On_side>(module, "On_side")
      .def(py::init([](const Plane& plane, const Point& P) {
        auto to_hull_kernel =
            CGAL::Cartesian_converter<Kernel, typename Hull::Kernel>{};
        return std::make_unique<On_side>(to_hull_kernel(plane),
                                         to_hull_kernel(P));
      }))
      .def("__call__", [](const On_side& self, const Point& P) {
        return self(
            CGAL::Cartesian_converter<Kernel, typename Hull::Kernel>{}(P));
      });
  ...
  py::class_<Surface>(module, "Surface")
      ...
      .def("as_arrays",
           [](Surface& self) { return as_numpy_arrays(self.mesh); })
      .def("simplify_connected_components",
           [](Surface& self) { simplify_connected(self); })
      ...
      //})
      .def("keep_connected",
           [](Surface& self, const On_side& mask) {
      ...
      .def("connected_components",
           [](Surface& self) { return connected_components_as_array(self); })
      .def("is_constrained_by", &Surface::is_constrained_by)
      .def("constraints",
           [](Surface& self) {
             auto result = py::list{};
             for (auto&& p : self.external_constraints_map) {
               result.append(py::make_tuple(Surface::human_readable(p.first),
                                            p.second.size()));
             }
             return result;
           })
      ...
      .def("mark_all_vertices_as_corners",
           &Surface::mark_all_vertices_as_corners);

  py::class_<Factory>(module, "Surface_factory")
      .def("__call__",
           [](Factory& self) { return self.make_unique_pointer(); });
  ...
  py::class_<Hull>(module, "Hull")
      ...
      .def(
          "boundaries",
          [](const Hull& self) {
            return py::make_iterator(begin(self.boundaries),
                                     end(self.boundaries));
          },
          py::keep_alive<0, 1>())
  ...
  module.def("simplify", [](Hull_boundary& B) { simplify_faces(B); });

  module.def("simplify", [](Surface& S) { simplify_faces(S.mesh); });
