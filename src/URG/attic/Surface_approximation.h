...
struct Surface_approximation {
  typedef Surface_type Surface;
  typedef typename Surface::Point Point;
  typedef typename CGAL::Kernel_traits<Point>::Kernel Kernel;
  typedef CGAL::Surface_mesh<Point> Tesselation;

  // TODO: Surface_mesh boundary edges ?
  ...
  auto intersect(const Line& l) const {
    ...
    if (points.empty()) return Result_type{};
    ...
    if (filtered_points.size() == 1) return Result_type{points.front()};
    return Result_type{Segment{points.front(), points.back()}};
  }
};
