...
import MeshTools.utils.edge as edge
...
hexahedron_centers = np.array(
    [np.mean(vertices[cell], axis=0) for cell in hexahedron]
)
...
splitted_edges = [
    ...
    for k, parts in enumerate(edge_parts[:edge_map.nb_edges]) if len(parts)!=0 
]
...
raw_mesh, original_cell = mesh.as_hybrid_mesh(
    convert_voxels=True, cell_centers=hexahedron_centers,
    face_centers=quadrangle_centers,
)
MT.to_vtu(
    raw_mesh, 'splitted_grid_edges',
