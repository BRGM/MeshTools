...
def create_and_distribute_mesh(
    ...
    local_face_cell = np.array([gid2lid[gci] for gci in face_cell], dtype=MT.idtype())
...
def test_distribution():
    shape = nx, ny, nz = 5, 5, 6
    mesh = create_and_distribute_mesh(
        MT.HexMesh.make,
        GT.grid2hexs(shape=shape, idtype=MT.idtype()),
        geometric_coloring,
    )
