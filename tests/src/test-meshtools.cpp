...
int main() {
  ...
  // std::ostream_iterator<MT::NodeId>(std::cout, " ")); for (auto&& i :
  // selected) { 	std::cout << i << " ";
  //}
  // std::cout << std::endl;
  ...
  for (auto&& P : cell_centers) std::cout << P << std::endl;
  auto face_centers = mesh.face_centers();
  std::cout << "Face centers: " << std::endl;
  for (auto&& P : face_centers) std::cout << P << std::endl;

  const auto& faces = connectivity.faces.nodes;
  auto boundaries = connectivity.boundary_faces();
  std::cout << "Face nodes:" << std::endl;
  for (auto&& face : faces) dump_nodes(face, std::cout) << std::endl;
  std::cout << "Boundaries faces:" << std::endl;
  std::copy(boundaries.begin(), boundaries.end(),
            std::ostream_iterator<MT::FaceId>(std::cout, " "));
  std::cout << std::endl;
  std::cout << "Face cells:" << std::endl;
  for (auto&& twins : connectivity.faces.cells) std::cout << twins << std::endl;
  std::cout << "Cell faces:" << std::endl;
  for (auto&& faces : connectivity.cells.faces) std::cout << faces << std::endl;
