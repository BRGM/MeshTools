...
def compute_surfaces(nb_layers, Lx, dxmin, Ly, dymin, bottom_left, cells, cell_layer, x, y, z, position):
    surfaces = np.empty((nb_layers, int(1+Lx/dxmin), int(1+Ly/dymin)), dtype='d')
    surfaces[:] = 9999.
    ix = (x - bottom_left[0]) / dxmin
    assert(all(np.modf(ix)[0]==0))
    ix = np.array(ix, dtype='i')
    ...
    for surface, squares in zip(surfaces, layer_nodes):
        ...
        for square in squares[not_small]:
            f = interpolate.interp2d(ix[square], jy[square],
                                     z[square], kind='linear')
...
def compute_thickness_layers(vertices, cells, cell_layer, nb_inoeuds, nb_jnoeuds, coord, bottom_left):
    ix, jy, x, y, z = coord
...
class ArretesNulles(object):
    ...
    def _get_faces_bot(self, faces_bots, faces_four, faces_three):
        ...
        uu, vv = np.where(toto[1] == np.repeat(indices[:, np.newaxis],
            5, axis=-1))
    ...
    def set_cells(self):
        for mesh in ['wedges', 'pyramids', 'tetras']:
            self.cells_nodes[mesh] = np.concatenate(self.cells_nodes[mesh], axis=0).tolist()
            self.cells_layers[mesh] = np.concatenate(self.cells_layers[mesh], axis=0).tolist()
            self.cells_affls[mesh] = np.concatenate(self.cells_affls[mesh], axis=0).tolist()
            self.cells_bots[mesh] = np.concatenate(self.cells_bots[mesh], axis=0).tolist()
    ...
    def compute_faces(self):
        ...
        for i, mesh in enumerate(['wedges', 'pyramids', 'tetras']):
            if mesh in newcells_faces.keys():
                toto = face_id[nmin[i]:nmax[i]].reshape((len(self.cells_nodes[mesh]), 
                    len(indices[mesh])), order='F')
                newcells_faces[mesh] = np.concatenate([newcells_faces[mesh],
                    toto], axis=1)
            else:
                newcells_faces[mesh] = face_id[nmin[i]:nmax[i]].reshape(
                        (len(self.cells_nodes[mesh]), len(indices[mesh])), order='F')
    ...
    def _add_wedges(self, mailles, mask):
        if np.any(mask):
            ...
            for j in iw:
                self.cells_nodes['wedges'].append(np.hstack([self.cells[mask][:, [j[0], j[1]]],
                    mailles[0][:, [0]], self.cells[mask][:, [j[2], j[3]]],
                    mailles[0][:, [1]]]))
    ...
    def one_vertice(self):
        ...
        for i in range(4):
            ...
            if np.any(m):
                for j in dic_iw[i]:
                    self.cells_nodes['wedges'].append(np.hstack([cells_ones[m][:, [j[0], j[1]]],
                        mailles[1][m][:, [0]], cells_ones[m][:, [j[2], j[3]]],
                        mailles[1][m][:, [1]]]))
                ...
                for j in dic_ip[i]:
                    self.cells_nodes['pyramids'].append(np.hstack([cells_ones[m][:, [j[0]]],
                            mailles[1][m][:, [0, 1]], cells_ones[m][:, [j[1], j[2]]]]))
    ...
    def two_vertices(self):
        ...
        if np.any(mask):
            ...
            for pos in positions:
                assert(not any(np.all(self.z[tcoins[:, pos]] == self.z[tcoins[:,[pos[0] + 4, pos[1] + 4]]],
                        axis = 1)))
            positions = [(0, 1), (0, 3), (3, 2), (1, 2)]
            for pos in positions:
                m = np.all(self.z[tcoins[:, pos]] == self.z[tcoins[:,[pos[0] + 4, pos[1] + 4]]],
                        axis = 1)
                iw = cells_wedges[pos]
    ...
    def three_vertices(self):
        ...
        for i in range(4):
            ...
            if np.any(m):
                for j in dic_it[i]:
                    self.cells_nodes['tetras'].append(np.hstack([mailles[1][m][:, [0, 1]],
                        cells_threes[m][:, [j[0], j[1]]]]))
                ...
                for j in dic_ip[i]:
                    self.cells_nodes['pyramids'].append(np.hstack([cells_threes[m][:, [j[0]]],
                        mailles[1][m][:, [0, 1]], cells_threes[m][:, [j[1], j[2]]]]))
