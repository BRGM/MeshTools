...
(dxmin, dymin, nb_inoeuds, nb_jnoeuds,
        coord, bottom_left, upper_right) = misc.check_mesh(
        vertices, cells, cell_layer)
ix, jy, x, y, z = coord
...
thicknesses = misc.compute_thickness_layers(
    vertices, cells, cell_layer,
    nb_inoeuds, nb_jnoeuds, coord, bottom_left) # =0 si empty = 1 si existe
...
surfaces_base = misc.compute_surfaces(
    nb_layers, Lx, dxmin, Ly, dymin,
    bottom_left, cells, cell_layer,
    x, y, z, 'base')
surfaces_top = misc.compute_surfaces(
    nb_layers, Lx, dxmin,
    Ly, dymin, bottom_left, cells, cell_layer,
    x, y, z, 'top')
surfaces=np.empty(
...
lat, lon = np.meshgrid(bottom_left[0] + np.arange(nx) * dxmin,
...
surfaces_ma = np.round(np.ma.masked_values(surfaces, 9999.), 2)
...
all_cells = np.hstack(
        [v.ravel().reshape((-1, 1)) for v in [
            node_number[:-1, :-1, 1:],  node_number[1:, :-1, 1:],
            node_number[:-1, 1:, 1:],   node_number[1:, 1:, 1:],
            node_number[:-1, :-1, :-1], node_number[1:, :-1, :-1],
            node_number[:-1, 1:, :-1],  node_number[1:, 1:, :-1]]])
a = np.reshape(all_cells, (surfaces.shape[0] - 1,
    surfaces.shape[1] - 1, surfaces.shape[2] - 1, 8))
...
for i in range(cell_is_good.shape[2]):
    ...
    surf_affl[couche] = np.where(surf_affl[couche] == 0, i + 1,
            surf_affl[couche])
new_surf_affl = np.zeros(cell_is_good.shape)
for i in range(cell_is_good.shape[2]):
    new_surf_affl[:, :, i] = np.where(surf_affl == i + 1,
            surf_affl, new_surf_affl[:, :, i])
new_surf_affl = new_surf_affl[cell_is_good]
# surfaces bottoms
surf_bot = np.zeros(cell_is_good.shape[:-1])
for i in range(cell_is_good.shape[2]-1, 0, -1):
    couche = cell_is_good[:, :, i]
    surf_bot[couche] = np.where(surf_bot[couche] == 0, i + 1,
            surf_bot[couche])
new_surf_bot = np.zeros(cell_is_good.shape)
for i in range(cell_is_good.shape[2]-1, 0, -1):
    new_surf_bot[:, :, i] = np.where(surf_bot == i + 1,
            surf_bot, new_surf_bot[:, :, i])
...
new_cell_col = np.repeat(new_cell_col[:, :, np.newaxis],
        cell_is_good.shape[2], axis=2)
new_cell_col = new_cell_col[cell_is_good]
# On enlève les arrêtes nulles
arnul = misc.ArretesNulles(new_cell_col, new_vertices, new_cells,
        new_cell_layer, new_surf_affl, new_surf_bot)
arnul.one_vertice()
...
arnul.set_cells()
typ = {'wedges':MT.Wedge, 'pyramids':MT.Pyramid,
        'tetras':MT.Tetrahedron, 'hexas':MT.Hexahedron}
