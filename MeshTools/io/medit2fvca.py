...
fcva_writer = {'standard':fvca.write,
...
aquifer_boundary_faces = (
    boundary_faces[np.where(mesh.cells_index[boundary_cells]==2)[0]]
)
assert np.all(mesh.cells_index[mesh.facecells[aquifer_boundary_faces,0]]==2)
xaquifer_faces = np.intersect1d(x_faces, aquifer_boundary_faces,
                                assume_unique=True)
...
vtkw.write_vtu(
    vtkw.vtu_doc(mesh.vertices, mesh.cells,
