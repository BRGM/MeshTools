...
from MeshTools.utils.edge import *
from MeshTools.io.face_id import is_same_face
# 2--3
# |  |
# 0--1
...
def pilar_edges(corner_ids, i, j, nz):
    edges = []
    for k in range(nz):
        for l in range(4):
...
def map_pilar_edges(edges, nodes):
    new_edges = {}
    for edge in edges:
        noeuds, noeuds_new = [], []
        for node in nodes:
        ...
        if len(noeuds_new) > 2:
            new_edges[edge] = [Edge(noeuds_new[u], noeuds_new[u+1])
...
def build_segs(i, j, corner_ids):
    segs1 = np.stack([corner_ids[i+1, j, :, 1],
        corner_ids[i+1, j+1, :, 3]], axis=-1)
    segs2 = np.stack([corner_ids[i+1, j, :, 5],
        corner_ids[i+1, j+1, :, 7]], axis=-1)
    segsY1 = np.where(segs1 == -1, segs2, segs1)
    segs3 = np.stack([corner_ids[i+1, j, :, 0],
        corner_ids[i+1, j+1, :, 2]], axis=-1)
    segs4 = np.stack([corner_ids[i+1, j, :, 4],
        corner_ids[i+1, j+1, :, 6]], axis=-1)
    segsY2 = np.where(segs3 == -1, segs4, segs3)
    segs1 = np.stack([corner_ids[i, j+1, :, 2],
        corner_ids[i+1, j+1, :, 3]], axis=-1)
    segs2 = np.stack([corner_ids[i, j+1, :, 6],
        corner_ids[i+1, j+1, :, 7]], axis=-1)
    segsX1 = np.where(segs1 == -1, segs2, segs1)
    segs3 = np.stack([corner_ids[i, j+1, :, 0],
        corner_ids[i+1, j+1, :, 1]], axis=-1)
    segs4 = np.stack([corner_ids[i, j+1, :, 4],
        corner_ids[i+1, j+1, :, 5]], axis=-1)
...
def build_segs_pils(self, segs1, segs2, i, j, u, v, pvertices):
    ...
    nodes1 = np.unique(pilar_edges(self.corner_ids, i+u, j+v, self.nz))
    asort = np.argsort(pvertices[nodes1][...,2])
    nodes1 = nodes1[asort]
    nodes2 = np.unique(pilar_edges(self.corner_ids, i+1, j+1, self.nz))
    asort = np.argsort(pvertices[nodes2][...,2])
    nodes2 = nodes2[asort]
    return segs_glo, nodes1, nodes2

def pilar_referential_tri(pts):
    z = pts[:, 2]
    kmin = np.argmin(z)
    kmax = np.argmax(z)
    e = pts[kmax]-pts[kmin]
...
def new_coord_tri(pilarpts, O, e): 
...
def map_edges_tri(segs_glo, nodes1, nodes2, pvertices):
    ...
    map_edges = map_pilar_edges(edges, nodes1)
    no = segs_glo[:,1][asort2]
    edges = [Edge(no[u], no[u+1]) for u in range(len(no)-1) if no[u] != no[u+1]]
    map_edges.update(map_pilar_edges(edges, nodes2))
    return map_edges
...
def triangulate(segs_glo, nodes1, nodes2, pvertices):
    ...
    Oback, uback = pilar_referential_tri(vertices1)
    Ofront, ufront = pilar_referential_tri(vertices2)
    ...
    uv, triangles, components, faces = PM.mesh(nv.astype('float64'), segs_tri)
    ...
    for old_node in old_nodes:
        ...
        if len(selec_nodes) > 2:
            map_edges[Edge(selec_nodes[0], selec_nodes[-1])] = [
                    Edge(node, selec_nodes[inode+1]) for inode, node in enumerate(selec_nodes[:-1])]
    return map_edges
...
def get_duplicate_face_id(new_faces_nodes):
    ...
    nmap = {old_id:[new_id] for old_id, new_id in zip(old_ids, new_ids) if old_id != new_id}
    return nmap
...
def map_old_new_faces_and_edges(self, cells_num, cells_comp, cells_faces, faces_tri,
        new_faces_nodes, num_face, segs_glo, map_faces, map_edges):
    for icell, num_cell in enumerate(np.unique(cells_num)):
        ...
        map_edges = do_map_edges(self, faces, num_cell, segs_glo, num_old_face, map_edges)
...
def solve_fault(self, i, j, axe, pvertices,  cells_faces,
        faces_nodes, map_edges, map_faces):
    segs1, segs2 = axe[0]
    ipil, jpil = axe[1]
    iverts = axe[2]
    iface1, iface2 = axe[3]
    segs_glo, nodes1, nodes2 = build_segs_pils(self,
            segs1, segs2, i, j, ipil, jpil, pvertices)
    nv, uv2, pvertices, components, triangles, faces_tri = triangulate(
            segs_glo, nodes1, nodes2, pvertices)
    sides = {
            1:[i, j, iface1, segs1],
            2:[i+ipil, j+jpil, iface2, segs2]
            }
    for key in sides.keys():
        w = sides[key]
        numcells = self.numcells[w[0], w[1]]
        cells_num, cells_comp = get_cells(uv2[:, iverts],
                triangles, components, w[3],
                pvertices[:, iverts], numcells
                )
        faces_nodes, map_faces, map_edges = map_old_new_faces_and_edges(
                self,
                cells_num, cells_comp, cells_faces, faces_tri,
                faces_nodes, w[2], segs_glo, map_faces, map_edges)
...
class PetrelGrid(object):
    def extract_data_from_files(self, mainfile, **kwargs):
        ...
        return pilars, actnum

    def build_grid(self, pilars, actnum=None):
        ...
        for i in [0, 4]:
            for k, pos in enumerate([
                ...
                for izc in range(self.nz):
                    # Vecteua directeur dirigé du haut vers le bas
                    # Calculé pour chaque pilier
                    dz = self.zcorn[:,:,izc,i+k] - pilars[pos+(2,)]
                    # Calcul des coordonnées x y de chaque noeud sur
                    # chaque pilier
        ...
        # Tests si certaines cellules ont moins de 4 arrêtes plates
        for i in range(1, 3):
    ...
    @classmethod
    def build_from_arrays(self, pilar_tops, pilar_bottoms, zcorn):
        grid = PetrelGrid()
        ncx, ncy, dim = pilar_tops.shape
        assert dim == 3
        nx, ny = ncx - 1, ncy - 1
        self.nx = nx
        self.ny = ny
        ...
        assert pilar_tops.shape==pilar_bottoms.shape
        pilars = np.zeros((ncx, ncy, 6), dtype=np.double)
        for i in range(ncx):
            for j in range(ncy):
                pilars[i, j, :3] = pilar_tops[i, j, :]
                pilars[i, j, 3:] = pilar_bottoms[i, j, :]
        grid.zcorn = zcorn
        grid.build_grid(pilars)
        self.permx = None
        self.permy = None
        self.permz = None
        return grid
    ...
    def process(self):
            ...
            for j in range(self.ny):
                for k in range(self.nz):
                    if self.mask_cells[i,j,k] == 1:
                        # Attention ! Les noeuds de l'hexagone sont 
        ...
        corner_ids = np.where(zpil == 9999., -1, corner_ids)
        self.corner_ids = corner_ids
        faces_nodes, cells_faces = set_faces_nodes(hexagons)
        self.faces_nodes = faces_nodes
        self.cells_faces = cells_faces
        return MT.idarray(hexagons), vertices, cells_faces, faces_nodes
            ...
            for j in range(self.ny):
                ...
                for axe in ['Y', 'X']:
                    ...
                    if detect_fault(segs1, segs2, pvertices):
                        (pvertices, new_faces_nodes,
                                map_edges, map_faces) = solve_fault(self,
                                i, j, axes[axe], pvertices,
                                cells_faces, new_faces_nodes, 
                                map_edges, map_faces
                                )
        faces_edges, edges = update_faces_edges(new_faces_nodes)
        new_faces_nodes = update_faces_nodes(new_faces_nodes, faces_edges, edges, map_edges)

        cells_faces, new_faces_nodes = passe_finale(cells_faces, new_faces_nodes, map_faces)
        map_duplicate_faces = get_duplicate_face_id(new_faces_nodes)
        cells_faces, new_faces_nodes = passe_finale(cells_faces, new_faces_nodes, map_duplicate_faces)
...
def import_eclipse_grid(filename):
    ...
    MT.to_vtu(MT.HexMesh.make(vertices, hexa), name)
    mesh = RM.RawMesh(
        vertices=vertices,
    ...
    MT.to_vtu(
        tetmesh, 'cells_as_tets_{0}'.format(name),
        celldata = {
    ...
    mesh = RM.RawMesh(
            ...
            face_nodes=face_nodes,
            cell_faces=cell_faces)
    hexahedron_centers = np.array(
        [np.mean(vertices[cell], axis=0) for cell in hexa])
    ...
    MT.to_vtu(
        raw_mesh, 'splitted_grid_edges_{0}'.format(name),
