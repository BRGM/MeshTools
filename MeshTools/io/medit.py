...
def _check_keyword(line, keyword, check=None, get_value_type=None):
    ...
    if check is not None:
        if type(check)(l[1].strip()) != check:
            raise Exception(
                'Expecting value %s for keyword "%s"' % (str(check), keyword)
            )
...
def _read_indexed_table(f, keyword, dtype):
    ...
    n = int(f.readline())
    data = np.array([f.readline().strip().split() for _ in range(n)], dtype=dtype)
...
def process(filename):
    ...
    with open(filename) as f:
        version = _check_keyword(
            f.readline(), "MeshVersionFormatted", get_value_type=int
        )
        if version == 1:
